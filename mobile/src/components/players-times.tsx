    ...
    return (
        <div className="px-4">
            ...
            <div className="flex flex-col mt-2">
                <button
                    onClick={onAddTime}
                    className="self-end rounded-md text-center bg-gradient-to-r from-orange-500 to-red-500 py-2 px-4"
                >
            ...
            {sort(times).map((t) => (
                <div key={t.id} className="flex justify-between my-2">
                    <span>{formatTime(new Date(t.time))}</span>
                    {t.player ? (
                        <div className="rounded-md text-center w-40 bg-gray-600 py-2 px-4">{t.player.number}</div>
                    ) : (
                        <button
                            onClick={() => setTimeStampToAssign(t.id)}
                            className="hover:bg-orange-500 hover:text-white hover:border-transparent rounded-md text-center w-40 border-dashed border-2 font-semibold py-2 px-4"
                        >
